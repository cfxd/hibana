...
export default function init({ el }) {
  const loadProductRecommendationsIntoSection = () => {
    ...
    const { productId } = productRecommendationsSection.dataset;
    // Read base URL from data attribute
    const { baseUrl } = productRecommendationsSection.dataset;
    // Read limit from data attribute
    const { limit } = productRecommendationsSection.dataset;
    // Build request URL
